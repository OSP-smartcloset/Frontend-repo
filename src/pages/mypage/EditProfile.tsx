...
function EditProfile() {
    ...
    useEffect(() => {
        // 로컬 스토리지에서 사용자 정보와 프로필 이미지 URL을 가져옵니다.
        const nickname = localStorage.getItem('nickname');
        const loginId = localStorage.getItem('loginId');
        const weight = localStorage.getItem('weight');
        const height = localStorage.getItem('height');
        const storedProfileImageUrl = localStorage.getItem('profile_image');
    ...
    return (
        <div className="p-4">
            ...
            <div className="flex mt-3">
                <button
                    className="w-full text-black border border-black rounded-lg mr-2"
                    onClick={openModal}
                >
                    프로필 이미지 변경
                </button>
            ...
            <div className="flex flex-col">
                <button
                    className="p-2 w-full mt-7 text-xl text-left"
                    onClick={handleInformChange}
                >
                    내 정보 수정
                </button>
